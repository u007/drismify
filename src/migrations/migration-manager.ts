...
export class MigrationManager {
  ...
  async getMigrationFiles(): Promise<MigrationFile[]> {
    ...
    for (const file of files) {
      ...
      if (match) {
        ...
        // If the filename doesn't match the expected format, log a warning
        if (this.options.debug) {
          console.log(`Warning: Migration file ${file} does not match the expected format YYYYMMDDHHMMSS_name.sql`);
        }
      }
    }
  ...
  async getPendingMigrations(): Promise<MigrationFile[]> {
    ...
    const pendingMigrations = migrationFiles.filter(file => !appliedMigrationNames.has(file.name));

    if (this.options.debug && pendingMigrations.length > 0) {
  ...
  private async recordMigration(migration: MigrationFile): Promise<void> {
    const query = `
      INSERT INTO ${this.migrationsTable} (name, timestamp, checksum, applied_at)
      VALUES (?, ?, ?, datetime('now'))
    `;
  ...
  async applyPendingMigrations(options: MigrationCommandOptions = {}): Promise<MigrationResult[]> {
    ...
    for (const migration of pendingMigrations) {
      ...
      if (!result.success && !options.force) {
        if (this.options.debug) {
          console.log(`Migration failed: ${migration.filename}`);
        }
        break;
      }
    }

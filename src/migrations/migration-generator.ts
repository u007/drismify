...
export class MigrationGenerator {
  ...
  async generateMigrationFromSchemaFile(
    ...
    // For now, we'll use a simple approach: check if the generated migration
    // would be identical to the latest migration
    const tempChanges = this.differ.diffSchemas([], ast);
    ...
    // Generate the migration SQL to compare
    const newMigrationSql = this.generateMigrationSql(tempChanges);
    ...
    // Compare the SQL content (ignoring comments and whitespace)
    const normalizedNew = this.normalizeSql(newMigrationSql);
    const normalizedLatest = this.normalizeSql(latestMigrationContent);

    if (normalizedNew === normalizedLatest) {
      if (this.options.debug) {
        console.log('Migration would be identical to latest migration, skipping');
      }
      return null;
    }
  ...
  /**
   * Normalize SQL for comparison by removing comments and extra whitespace
   */

...
export function applyExtension(client: any, extension: Extension): any {
  ...
  if (extension.model) {
    applyModelExtension(client, extension.model);
  }
  
  // Apply client extensions
  if (extension.client) {
    applyClientExtension(client, extension.client);
  }
  
  // Apply query extensions
  if (extension.query) {
    applyQueryExtension(client, extension.query);
  }
  
  // Apply result extensions
  if (extension.result) {
    applyResultExtension(client, extension.result);
  }
    ...
    for (const modelName of modelNames) {
      ...
      for (const queryName in allModelQueries) {
        ...
        model[queryName] = function(...args: any[]) {
          const modifiedArgs = queryModifier(args[0] || {});
